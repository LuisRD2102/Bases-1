...
$("#tablaPersona tr").click(function(){
    ...
    if (tipoPersona.localeCompare("Cliente")==0){
        btnBeneficiario.classList.add('desactivado');
        btnTitular.classList.remove('desactivado');
        btnBeneficiario.disabled = true;
        btnTitular.disabled = false;
    }
    else{        
        btnTitular.classList.add('desactivado');
        btnBeneficiario.classList.remove('desactivado');
        btnBeneficiario.disabled = false;
        btnTitular.disabled = true;
    }
});
...
function agregarBeneficiario(){
    ...
    $(newTr).children("td:last").html(newButtonHTML);
    items.push(newTr);
    newTr.appendTo($("#tablaBeneficiario"));
    $("#tablaBeneficiario tr:last").removeClass('selected');
}
